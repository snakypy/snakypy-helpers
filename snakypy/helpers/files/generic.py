...
def read_file(file_path: str, split: bool = False) -> Union[str, List[str]]:
    ...
    >>> snakypy.helpers.files.read_file(file)
...
def backup_file(
    ...
    >>> import snakypy
    >>> snakypy.helpers.files.backup_file("/tmp/file.txt", "/tmp/file.txt")
    >>> snakypy.helpers.files.backup_file("/tmp/file.txt", "/tmp/file.txt", extension=False)
    >>> snakypy.helpers.files.backup_file("/tmp/file.txt", "/tmp/file2.txt")
    >>> snakypy.helpers.files.backup_file("/tmp/file.txt", "/tmp/file2.txt", date=True)

    or using from

    >>> from snakypy.helpers.files import backup_file
    >>> backup_file("/tmp/file.txt", "/tmp/file.txt")
    >>> backup_file("/tmp/file.txt", "/tmp/file.txt", extension=False)
    >>> backup_file("/tmp/file.txt", "/tmp/file2.txt")
    >>> backup_file("/tmp/file.txt", "/tmp/file2.txt", date=True)
